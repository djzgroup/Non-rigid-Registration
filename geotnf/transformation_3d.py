...
class SynthPairTnf(object):
    ...
    def __init__(self, use_cuda=True, geometric_model='affine', crop_factor=1.0, output_size=(240, 240),
                 padding_factor=0.5):
...
class TpsGridGen(Module):
    def __init__(self, out_h=240, out_w=240, out_d=240, use_regular_grid=True, grid_size=3, reg_factor=0,
                 use_cuda=True):
        ...
        self.grid_X, self.grid_Y, self.grid_Z = np.meshgrid(np.linspace(-1, 1, out_w), np.linspace(-1, 1, out_h),
    ...
    def compute_L_inverse(self, X, Y, ZZ):
        ...
        P_dist_squared = torch.pow(Xmat - Xmat.transpose(0, 1), 2) + torch.pow(Ymat - Ymat.transpose(0, 1),
                                                                               2) + torch.pow(
            Zmat - Zmat.transpose(0, 1), 2)
    ...
    def apply_transformation(self, theta, points):
        ...
        points_Z_for_summation = points[:, :, :, :, 2].unsqueeze(3).unsqueeze(4).expand(
            points[:, :, :, :, 2].size() + (1, self.N))
